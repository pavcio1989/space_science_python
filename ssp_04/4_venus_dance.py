...
# string
INIT_TIME_UTC_STR = datetime.datetime(year=2025, month=1, day=1) \
    .strftime('%Y-%m-%dT%H:%M:%S')
END_TIME_UTC_STR = datetime.datetime(year=2025, month=6, day=1) \
    .strftime('%Y-%m-%dT%H:%M:%S')
...
INNER_SOLSYS_DF.loc[:, 'PHOTOGENIC'] = \
    INNER_SOLSYS_DF.apply(lambda x: 1 if (x['EARTH_VEN2SUN_ANGLE'] > 30.0)
                                         ...
                                         & (x['EARTH_MOON2VEN_ANGLE'] < 10.0) else 0, axis=1)
...
plt.savefig('../_images/VENUS_SUN_MOON.png', dpi=300)
