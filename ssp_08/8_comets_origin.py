...
# for P type ...
P_TYPE_DF = pd.read_sql('SELECT APHELION_AU, INCLINATION '
                        'FROM comets_main WHERE ORBIT_TYPE="P"', CON)

# ... and C type comets. For this type: include also the eccentricity
C_TYPE_DF = pd.read_sql('SELECT APHELION_AU, INCLINATION, ECCENTRICITY '
...
ax.scatter(C_TYPE_DF[C_TYPE_DF['ECCENTRICITY'] < 1]['APHELION_AU'],
           C_TYPE_DF[C_TYPE_DF['ECCENTRICITY'] < 1]['INCLINATION'],
...
plt.savefig('../_images/comets_scatter_plot_Q_i.png', dpi=300)
...
ax.hist(P_TYPE_DF['INCLINATION'],
        bins=nr_of_bins(P_TYPE_DF['INCLINATION']),
        density=True, color='tab:orange', alpha=0.5, label='P Type')

ax.hist(C_TYPE_DF['INCLINATION'],
        bins=nr_of_bins(C_TYPE_DF['INCLINATION']),
...
# Save the figure
plt.savefig('../_images/comets_kde_incl_.png', dpi=300)
