...
# for P type ...
P_TYPE_DF = pd.read_sql('SELECT NAME, SEMI_MAJOR_AXIS_AU, INCLINATION, '
                        'ECCENTRICITY FROM comets_main WHERE ORBIT_TYPE="P"',
                        con)

# ... and C type comets. For this type: set the eccentricity smaller 1 (bound
# orbits)
C_TYPE_DF = pd.read_sql('SELECT NAME, SEMI_MAJOR_AXIS_AU, INCLINATION, '
...
print(f"Semi major axis of Jupyter equals {A_JUPITER_AU} AU")
...
def tisr_jup(a, i, e):
    return (A_JUPITER_AU / a) + 2 * np.cos(i) \
...
PERC_P_TYPE_JFCS = len(P_TYPE_DF.loc[(P_TYPE_DF["TISSERAND_JUP"] > 2) & (P_TYPE_DF["TISSERAND_JUP"] < 3)]) \
...
# Save the figure
plt.savefig('../_images/comets_kde_tisserand_jup.png', dpi=300)
