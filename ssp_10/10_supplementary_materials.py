...
print(f"Semi-major axis of Jupyter is {A_JUPITER_AU} AU")
...
# object)
def tisserand_jup(a, i, e):
    return (A_JUPITER_AU / a) \
        + 2 * np.cos(i) \
...
pathlib.Path('../_images/ssp_10/temp/').mkdir(parents=True, exist_ok=True)
...
for a_for in tqdm(range(len(A_ARRAY))):
    ...
    plt.savefig('../_images/ssp_10/temp/'+str(a_for+1).zfill(3)+'.png', dpi=100)

# In this part all figures are merged into one animation (GIF)

FILE_NAMES = glob.glob('../_images/ssp_10/temp/*.png')
