...
print("""Position (components) of the Solar System Barycentre w.r.t the centre of the Sun (at initial time):\n
    X = %s km \n
    Y = %s km \n
...
print("""Distance between the Solar System Barycentre w.r.t the centre of the Sun (at initial time): \n
    d = %s km""" % round(np.linalg.norm(SSB_WRT_SUN_POSITION[0])))

# We want to visualise the results, to a get feeling of the movement. Is the
# movement somehow interesting and / or significant?

# Using km are not intuitive. AU would scale it to severely. Since we compute
...
print(f"Scaled distance between the Solar System Barycentre \n"
      f"w.r.t the centre of the Sun (at initial time): {round(np.linalg.norm(SSB_WRT_SUN_POSITION_SCALED[0]))}")
...
plt.savefig('../_images/SSB_WRT_SUN.png', dpi=300)
