...
# Planet Center
DL_PATH = 'raw_data/'
DL_URL = 'https://www.minorplanetcenter.net/Extended_Files/allcometels.json.gz'
...
c_df = pd.read_json('raw_data/allcometels.json.gz', compression='gzip')
c_df.drop(c_df.index[[0, 1]], inplace=True)
...
c_df.loc[:, 'EPOCH_UTC_TIME'] = \
    c_df['Day_of_perihelion'].apply(lambda x: (PRE_TIME + datetime.timedelta(days=x)).strftime('%H:%M:%S'))
...
HALE_BOPP_ORB_ELEM = [spiceypy.convrt(HALE_BOPP_DF['Perihelion_dist'].iloc[0], 'AU', 'km'),
...
c_df.loc[:, 'SEMI_MAJOR_AXIS_AU'] = \
    c_df.apply(lambda x: x['Perihelion_dist'] / (1.0 - x['e']) if x['e'] < 1 else np.nan,
               axis=1)

# ... as well as the apohelion (if applicable)
c_df.loc[:, 'APHELION_AU'] = \
    c_df.apply(lambda x: (1.0 + x['e']) * x['SEMI_MAJOR_AXIS_AU'] if x['e'] < 1 else np.nan,
               axis=1)
