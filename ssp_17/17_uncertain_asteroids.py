...
# Save the figure
plt.savefig('../_images/ssp_17/asteroid_resample_e_q.png', dpi=300)
...
DIST_MAT = cdist(np.array(ast_2020_jx1_df['POS_VEC_KM'].values.tolist()),
                 np.array(ast_2020_jx1_df['POS_VEC_KM'].values.tolist()))
...
ast_2020_jx1_df.loc[:, 'ECLIP_LONG_DEG'] = \
    ast_2020_jx1_df['POS_VEC_WRT_EARTH_KM'].apply(lambda x: np.degrees(spiceypy.recrad(x)[1]))
ast_2020_jx1_df.loc[:, 'ECLIP_LAT_DEG'] = \
    ast_2020_jx1_df['POS_VEC_WRT_EARTH_KM'].apply(lambda x: np.degrees(spiceypy.recrad(x)[2]))
